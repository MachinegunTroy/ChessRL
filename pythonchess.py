...
if __name__ == "__main__":
    ...
    while not env.board.is_game_over():
        ...
        # Encode the move into our new action space.
        # Determine the promotion flag if applicable.
        piece = env.board.piece_at(move.from_square)
        promotion_flag = 0  # Default: no promotion.
        if piece and piece.piece_type == chess.PAWN:
            rank = chess.square_rank(move.to_square)
            if (piece.color == chess.WHITE and rank == 7) or (piece.color == chess.BLACK and rank == 0):
                # For demonstration, choose a random promotion option among 1,2,3,4.
                promotion_flag = random.randint(1, 4)
        base_move = move.from_square * 64 + move.to_square
        action = promotion_flag * 4096 + base_move
